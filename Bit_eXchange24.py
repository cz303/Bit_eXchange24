...
def next(message):
	if message.text == 'Обменять \U0001F4B8':
		...
		bot.send_message(message.chat.id, "\U0001F4A1 <b>Важная информация:</b> \n\n\U000025FE Наш бот - @Bit_eX24bot \n\U000025FE При сбое бота - напишите команду - /start \n\U000025FE Транзакция BitCoin отправляется нами максимально возможным приоритетом (1 блок), остальное зависит от самой BTC сети. Ею мы не управляем и никто не управляет \n\U000025FE Не принимаются жалобы на недействующие коды EXMO, если они были переданы третьим лицам или активированы не на сайтах платежных систем соответственно \n\U000025FE Если Вы отправили вместо QIWI на мобильный телефон деньги, то никто вам их не вернет, т.к. мы тоже их не получим. Так же не рассматриваются платежи без комментария \n\U000025FE Всю важную информацию и отзывы мы публикуем в нашем новостном канале - @Bit_eX24 \n\U000025FE On-line поддержка - @Bit_eX24sup,  время работы оператора поддержки с 8 утра до 22 вечера \n\U000025FE Бесплатный  прокси при блокировке Telegram https://t.me/proxy?server=142.93.100.244&port=443&secret=086300a794a285f1ceb60fdaecb81cac")
		second_menu(message)
